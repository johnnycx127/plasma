...
import (
	...
	tutil "github.com/kyokan/plasma/test_util"
)
...
func Test_OneTransactionMatches(t *testing.T) {
	...
	from := tutil.RandomAddress()
	to := tutil.RandomAddress()
	amount := big.NewInt(0)
	client := new(mocks.Client)
	client.On("SignData", mock.AnythingOfType("*common.Address"), mock.AnythingOfType("[]uint8")).Return(tutil.RandomSig(), nil)
	...
	for i := 0; i < size; i++ {
		...
		transactions[i] = chain.Transaction{
			Input0: tutil.RandomInput(),
			Input1: tutil.RandomInput(),
		}
		if outputIdx == false {
			transactions[i].Output0 = output
			transactions[i].Output1 = tutil.RandomOutput()
		} else {
			transactions[i].Output1 = output
			transactions[i].Output0 = tutil.RandomOutput()
		}
	}
